...
func main() {
	...
	appleImage := rl.LoadImage("assets/apple.png")
	rl.ImageResize(appleImage, 25, 25) // Resize the apple image to a smaller size
	defer rl.UnloadImage(appleImage)
	...
	for !rl.WindowShouldClose() {
		...
		if !gameOver {
			...
			if len(Apples) == 0 {
				...
				current_appleImage := rl.LoadTextureFromImage(appleImage)
				current_apple := Apple{screenWidth, apple_loc, current_appleImage}
				Apples = append(Apples, current_apple)
			}

			for io, current_apple := range Apples {
				rl.DrawTexture(current_apple.image, current_apple.posX, current_apple.posY, rl.White)
				...
				if rl.CheckCollisionRecs(rl.NewRectangle(float32(x_coords), float32(y_coords), 34, 24), rl.NewRectangle(float32(current_apple.posX), float32(current_apple.posY), float32(current_apple.image.Width), float32(current_apple.image.Height)) ) {
					Apples = append(Apples[:io], Apples[io+1:]...)
					score++
